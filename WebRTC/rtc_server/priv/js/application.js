    ...
    class myWebsocketHandler {
        ...
        setupSocket() {
            ...
            this.socket.onopen = () => {
                ...
                let dataChannel = rtcPeerConnection.createDataChannel('dc', dataChannelConfig);
                ...
                const onOfferCreated = (description) => {
                    ...
                    this.socket.send(JSON.stringify({ type: 'offer', payload: description }));
                }

                const onIceCandidate = (event) => {
                    console.log(event.target.iceGatheringState)
                    if (event && event.candidate) {
                        this.socket.send(JSON.stringify({ type: 'candidate', payload: event.candidate }));
                    }
                }
